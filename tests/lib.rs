...
mod tests {
    ...
    #[test]
    pub fn test_solver(){
        let solver = init_cadical_solver();
        let nb_v = parse_dimacs("tests/test.cnf", solver);
        let ret = run_cadical(solver);
        match ret {
            //cadical returns 10 for feasible.
            10 => { 
                let sol = get_cadical_solution(solver, nb_v); 
    ...
    pub fn parse_dimacs(path: &str, solver : *mut CCaDiCaL) -> usize {
        ...
        for line in buffered.lines() {
            ...
            if l.contains("p") && l.contains("cnf") {
                ...
                add_clause_to_cadical_solver(solver, v_clause);
            }
        }
        nb_v
    }
